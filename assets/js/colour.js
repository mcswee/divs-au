...
function initializeCardData() {
    document.querySelectorAll('.colour-card').forEach(card => {
        ...
        // Grab the pre-calculated WCAG values from the HTML data attributes
        const vsWhite = cleanValue(card.dataset.wcagVsFff);
        const vsBlack = cleanValue(card.dataset.wcagVs000);

        if (swatch) {
            // Apply background from hex
            swatch.style.backgroundColor = card.dataset.hex;
            
            // Set internal swatch text/icon colour based on best contrast
            // (Even if you don't have text there yet, this future-proofs it)
            swatch.style.color = vsWhite > vsBlack ? '#ffffff' : '#000000';
        }
        
        // Note: .card-info (the white box) remains untouched to preserve your layout
    });
}
...
function sortGrid(criterion, ascending = true) {
    ...
    cards.sort((a, b) => {
        let valA, valB;
        // Convert "WCAG vs fff" to "wcag-vs-fff" to match dataset keys
        const key = criterion.toLowerCase().replace(/[\s_]+/g, '-');

        switch(criterion) {
            case 'Family':
                // Check 'fam' attribute from your CSV
                valA = familyOrder[a.dataset.fam] || 99;
                valB = familyOrder[b.dataset.fam] || 99;
                break;
...
document.addEventListener('DOMContentLoaded', () => {
    // Initial setup of swatch colours and contrast
    initializeCardData();
    ...
    if (sortSelect) {
        sortSelect.addEventListener('change', function() {
            ...
            // Trigger View Transition API if available
            if (document.startViewTransition) {
                document.startViewTransition(() => sortGrid(this.value, isAscending));
            } else {
    ...
    const filterGrid = () => {
        ...
        document.querySelectorAll('.colour-card').forEach(c => {
            const nameMatch = c.dataset.name.toLowerCase().includes(term);
