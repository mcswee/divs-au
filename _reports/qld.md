---
layout: redistribution
state: "Queensland"
state_id: qld
year: 2025
title: "Queensland suggested redistribution"
status: "active"
category: reports
aec_url: "https://www.aec.gov.au/redistributions/2025/qld/"
---
The redistribution of Queensland officiallycommemved on 28 July 2026. Electoral data are expected to be available in Septeemet, on the day public suggestions are announced. In the meantime, I have launched the interactive <a href ="/tool">redistribution tool</a>. The population data for Queensland is outdated, based on the original 2018 redistribution, and currently missing some data where SA1 boundaries and regions have changed.

The Australian Electoral Commissioner has, on 23 July 2026, completed the <a href ="https://www.aec.gov.au/redistributions/entitlement/current-entitlement.html?utm_source=miragenews&utm_medium=miragenews&utm_campaign=news">entitlement determination</a>, and established that Queensland continues to be entitled to 30 seats.
